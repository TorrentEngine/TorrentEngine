...
import pluginsimpl.local.disk.DiskManagerChannelImpl;

// import torrentlib.contentnetwork.ContentNetwork;
...
import torrentlib.util.PlatformTorrentUtils2;
import torrentlib.util.ExternalStimulusHandler;
import torrentlib.util.ExternalStimulusListener;
...
DownloadManagerEnhancer
{
		...
		TorrentEngineCore	_core )
	{
		...
		boolean	tag_all = false; // initAutoTag();
	...
	progressiveActivated()
	{
		synchronized( this ){
			...
			if ( pa_timer == null ){
				...
				pa_timer =
					SimpleTimer.addPeriodicEvent("DownloadManagerEnhancer:speedChecker",
						TICK_PERIOD,
	...
	public DownloadManager findDownloadManager(String hash) {
		synchronized (download_map) {
			...
			for (Iterator<DownloadManager> iter = download_map.keySet().iterator(); iter.hasNext();) {
				...
				TOTorrent torrent = dm.getTorrent();
				if (PlatformTorrentUtils2.isContent(torrent, true)) {
					String thisHash = PlatformTorrentUtils2.getContentHash(torrent);
...
//			if ( torrent != null ){
//
//				boolean is_vhdn = PlatformTorrentUtils2.getContentNetworkID(torrent) == ContentNetwork.CONTENT_NETWORK_VHDNL;
//
//				String content_type = PlatformTorrentUtils2.getContentType(torrent);
//
...
//				}
//
//				if ( PlatformTorrentUtils2.isFeaturedContent( torrent )){
//
