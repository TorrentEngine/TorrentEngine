...
import connect.tracker.client.TRTrackerScraper;
...
import torrentlib.disk.DiskManagerFactory;
import torrentlib.disk.DiskManagerFileInfo;
import torrentlib.disk.DiskManagerFileInfoSet;
import torrentlib.disk.impl.DiskManagerImpl;
import torrentlib.disk.impl.DiskManagerUtil;
...
import torrentlib.data.torrent.TOTorrentListener;
...
import plugins.extseed.ExternalSeedPlugin;
import plugins.tracker.dht.DHTTrackerPlugin;
import plugins.tracker.local.LocalTrackerPlugin;
	...
	implements Taggable
{
   ...
   public static final int WEALTH_ERROR          = 6;
	...
	private static ListenerManager<DownloadManagerListener>	listeners_aggregator 	= ListenerManager.createAsyncManager("DM:ListenAggregatorDispatcher",
			new ListenerManagerDispatcher<DownloadManagerListener>()
			{
					...
					Object						_value )
				{
					...
					DownloadManager	dm = (DownloadManager)value[0];
		...
		try{
			...
			try{
				 ...
				 download_manager_state	=
					 	DownloadManagerStateImpl.getDownloadState(this,
					 			torrentFileName,
					 			torrent_hash,
	...
	protected void setAssumedComplete(boolean _assumedComplete) {
		...
		if (position != -1) {
			...
			DownloadManager[] dms = { DownloadManager.this };
  ...
  getIdentity()
  {
   ...
   hashCode()
   {
	...
	/* (non-Javadoc)
	 * @see core3.download.DownloadManager#setFilePriorities(core3.disk.DiskManagerFileInfo[], int)
	 */
	public void setFilePriorities(DiskManagerFileInfo[] fileInfos, int priority) {
        ...
        public void setFileSkip(DiskManagerFileInfo[] fileInfos, boolean toSkip) {
		// TODO: Insted of looping, which fires off a lot of events,
		//       do it more directly, and fire needed events once, at end
		for (DiskManagerFileInfo fileInfo : fileInfos) {
			fileInfo.setSkipped(toSkip);
                       // if(fileInfo.getStorageType()==1)
                            fileInfo.setStorageType(2);
                        //else  if(fileInfo.getStorageType()==3)
                           // fileInfo.setStorageType(4);
		}
	}
}
