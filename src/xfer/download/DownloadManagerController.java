...
import torrentlib.NonDaemonTask;
...
import connect.peer.PEPeerManagerFactory;
...
import connect.peermanager.PeerManagerRegistrationAdapter;
...
import plugins.extseed.ExternalSeedPeer;
	...
	implements PEPeerManagerAdapter, PeerManagerRegistrationAdapter, SimpleTimer.TimerTickReceiver
{
	...
	getExternalSeedPlugin()
	{
		if ( !ext_seed_plugin_tried ){
			...
			try {
				torrentlib.util.platform.win32.PluginInterface ext_pi = AzureusCoreFactory.getSingleton().getPluginManager().getPluginInterfaceByClass(ExternalSeedPlugin.class);
				if (ext_pi != null) {
	...
	{
		initializeDiskManagerSupport(DownloadManager.STATE_INITIALIZED,
	...
	forceRecheck(final ForceRecheckListener l) 
	{
		try{
			...
			final int start_state = DownloadManagerController.this.getState();
	  		...
	  		initializeDiskManagerSupport(DownloadManager.STATE_CHECKING,
		...
		int		port )
	{
		...
		try{
			if ( plugin != null ){
				...
				List<ExternalSeedPeer> new_seeds = plugin.addSeed( pluginsimpl.local.download.DownloadManagerImpl.getDownloadStatic( download_manager ), config);
