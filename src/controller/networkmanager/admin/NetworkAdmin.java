...
import torrentlib.instancemanager.AZInstance;
import torrentlib.instancemanager.AZInstanceManager;
import torrentlib.instancemanager.AZInstanceManagerListener;
import torrentlib.instancemanager.AZInstanceTracked;
import controller.networkmanager.NetworkConnectionBase;
import controller.networkmanager.NetworkManager;
import controller.networkmanager.TransportBase;
import controller.networkmanager.TransportStartpoint;
import controller.networkmanager.http.HTTPNetworkManager;
...
import comm.proxy.socks.AESocksProxy;
import comm.proxy.socks.AESocksProxyFactory;
import torrentlib.CopyOnWriteList;
import torrentlib.util.NetUtils;
import plugins.upnp.UPnPPlugin;
import plugins.upnp.UPnPPluginService;
import controller.networkmanager.Transport;
import torrentlib.TorrentEngineCore;

public class
NetworkAdmin implements AEDiagnosticsEvidenceGenerator
{
		...
		AzureusCoreFactory.addCoreRunningListener(new AzureusCoreRunningListener()
			{
				...
				{
					try{
						Class.forName( "core.networkmanager.admin.impl.swt.NetworkAdminSWTImpl" ).getConstructor(new Class[]{ TorrentEngineCore.class, NetworkAdmin.class }).newInstance(new Object[]{ core, NetworkAdmin.this });
		...
		implements NetworkAdminNetworkInterface
	{
			...
			implements NetworkAdminNetworkInterfaceAddress
		{
			...
			getRoute(
				...
				throws NetworkAdminException
			{
				return( NetworkAdmin.this.getRoute( address, target, max_millis, listener));
			}
			...
			pingTarget(
				...
				throws NetworkAdminException
			{
				return( NetworkAdmin.this.pingTarget( address, target, max_millis, listener));
			}
