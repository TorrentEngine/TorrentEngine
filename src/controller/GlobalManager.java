...
import xfer.download.DownloadManagerAdapter;
...
import connect.tracker.util.TRTrackerUtils;
import connect.tracker.util.TRTrackerUtilsListener;
...
import torrentlib.data.tag.TagDownloadWithState;
import torrentlib.data.tag.TagTypeWithState;
...
public class GlobalManager
   extends DownloadManagerAdapter
	implements AzureusCoreComponent, TaggableResolver, AEDiagnosticsEvidenceGenerator
{
		...
		String savePath)
  {
  	...
  	return addDownloadManager(fileName, null, savePath, DownloadManager.STATE_QUEUED, true);
  }
  	...
  	boolean	immediate )
  {
    ...
    //    if(Boolean.getBoolean("debug")) return;
  	...
  	try{
	    ...
	    Collections.sort(managers_temp,
	    	new Comparator ()
	    	{
	    		public final int
  ...
  informDestroyInitiated()
  {
	...
	{
		listeners_and_event_listeners.dispatch(LDT_EVENT,
