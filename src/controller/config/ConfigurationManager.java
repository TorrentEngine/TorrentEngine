...
import torrentlib.ConcurrentHashMapWrapper;
...
import controller.config.COConfigurationManager.ParameterVerifier;
import controller.config.COConfigurationManager.ResetToDefaultsListener;
...
{
  final public static String TorrentDirectory = System.getProperty("user.home") + "/.atdown/";
  ...
  private List<ResetToDefaultsListener>	reset_to_def_listeners = new ArrayList<>();
  	...
  	Map data )
  {
	  ...
	  propertiesMap	= new ConcurrentHashMapWrapper<>( data );
  }
	  ...
	  synchronized( exported_parameters ){
		  ...
		  try{
			  ...
			  Set<String>	exported_keys = new HashSet<>();
                          ...
                          exported_parameters.values().stream().forEach((entry) -> {
                              String	key		= entry[0];
