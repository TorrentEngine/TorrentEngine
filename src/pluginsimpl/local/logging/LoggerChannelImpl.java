	...
	implements LoggerChannel
{
	...
	isEnabled()
	{
		return torrentlib.util.logging.Logger.isEnabled();
	}
	...
	public void log(Object[] relatedTo, int log_type, String data) {
		...
		if (isEnabled() && !no_output) {
			data = "[" + name + "] " + data;
			torrentlib.util.logging.Logger.log(new LogEvent(relatedTo, LOGID,
					LogTypePluginToCore(log_type), data));
		}
	}
  ...
  public void log(String str, Throwable error) {
		...
		if (!no_output) {
			...
			torrentlib.util.logging.Logger.log(event);
		}
	}
	...
	protected void logAlert(int alert_type, String message, boolean repeatable) {
		...
		if (!no_output) {
			...
			torrentlib.util.logging.Logger.log(new LogAlert(repeatable, at,
					message));
		}
	}
		...
		Throwable 	e )
	{
		...
		if ( !no_output ){
			torrentlib.util.logging.Logger.log(new LogAlert(
