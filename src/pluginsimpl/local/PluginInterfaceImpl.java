...
import plugins.torrent.TorrentManager;
import plugins.download.DownloadManager;
import plugins.download.DownloadException;
import plugins.dht.mainline.MainlineDHTManager;
import torrentlib.util.platform.win32.PluginManager;
import torrentlib.util.platform.win32.PluginConfig;
...
import plugins.utils.ShortCuts;
...
import pluginsimpl.local.dht.mainline.MainlineDHTManagerImpl;
import pluginsimpl.local.tracker.TrackerImpl;
import pluginsimpl.local.ui.UIManagerImpl;
import pluginsimpl.local.ui.config.ConfigSectionRepository;
import pluginsimpl.local.ui.config.ParameterRepository;
import pluginsimpl.local.ui.config.PluginConfigUIFactoryImpl;
import pluginsimpl.local.ui.config.ConfigSectionHolder;
import pluginsimpl.local.utils.UtilitiesImpl;
import pluginsimpl.local.utils.ShortCutsImpl;
import java.util.*;
import java.io.File;
import java.net.URL;
...
import pluginsimpl.local.deprecate.PluginDeprecation;
...
import pluginsimpl.local.ipc.IPCInterfaceImpl;
import pluginsimpl.local.ipfilter.IPFilterImpl;
import pluginsimpl.local.logging.LoggerImpl;
import pluginsimpl.local.messaging.MessageManagerImpl;
import pluginsimpl.local.network.ConnectionManagerImpl;
import pluginsimpl.local.sharing.ShareManagerImpl;
import pluginsimpl.local.torrent.TorrentManagerImpl;
import plugins.ipc.IPCInterface;
import plugins.ipfilter.IPFilter;
import plugins.tracker.Tracker;
import plugins.ui.config.Parameter;
import plugins.ui.config.PluginConfigUIFactory;
import plugins.platform.PlatformManager;
import plugins.clientid.ClientIDManager;
...
{
	private static final LogIDs LOGID = torrentlib.util.logging.LogIDs.PLUGIN;
  	...
  	throws PluginException
  {
	  ...
	  if ( !(	caller_class.equals( "pluginsimpl.local.PluginInitializer" ) ||
			  	caller_class.equals( "pluginsimpl.local.PluginInterfaceImpl" ))){
	  ...
	  if ( class_name == null || !class_name.equals( "pluginsimpl.local.PluginInterfaceImpl" )){
		...
		String		val )
	{
  		...
  		if ( ! ( plugin.getClass().getName().startsWith( "org.gudy") || plugin.getClass().getName().startsWith( "com.aelitis."))){
	  		...
	  		if ( str.equalsIgnoreCase( "plugin.id" ) || str.equalsIgnoreCase("plugin.version" )){
	  			...
	  			if (torrentlib.util.logging.Logger.isEnabled())
						torrentlib.util.logging.Logger
	 		...
	 		if ((!initialising ) && key instanceof String ){
	  			...
	  			String	k_str = (String)key;
	  	 		...
	  	 		if ( k_str.equalsIgnoreCase( "plugin.id" ) || k_str.equalsIgnoreCase("plugin.version" )){
	  	 			...
	  	 			if (torrentlib.util.logging.Logger.isEnabled())
							torrentlib.util.logging.Logger.log(new LogEvent(LOGID,
