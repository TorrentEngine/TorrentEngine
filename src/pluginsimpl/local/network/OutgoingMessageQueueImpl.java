...
import plugins.network.OutgoingMessageQueueListener;
...
public class OutgoingMessageQueueImpl implements OutgoingMessageQueue {
  private final controller.networkmanager.OutgoingMessageQueue core_queue;
  private final HashMap registrations = new HashMap();
  
  
  protected OutgoingMessageQueueImpl( controller.networkmanager.OutgoingMessageQueue core_queue ) {
  ...
  public void registerListener( final OutgoingMessageQueueListener listener ) {
    controller.networkmanager.OutgoingMessageQueue.MessageQueueListener core_listener =
      new controller.networkmanager.OutgoingMessageQueue.MessageQueueListener() {
      
        public boolean messageAdded( connect.peer.messaging.Message message ) {
          if( message instanceof MessageAdapter ) {
        ...
        public void messageQueued( connect.peer.messaging.Message message ) {  /*nothing*/  }
        public void messageRemoved( connect.peer.messaging.Message message ) {  /*nothing*/  }

        public void messageSent( connect.peer.messaging.Message message ) {
          if( message instanceof MessageAdapter ) {
            //the message must have been originally created by plugin encoder and wrapped
            //so just use original plugin message...i.e. unwrap out of MessageAdapter
  ...
  public void deregisterListener( OutgoingMessageQueueListener listener ) {
    ...
    controller.networkmanager.OutgoingMessageQueue.MessageQueueListener core_listener =
      (controller.networkmanager.OutgoingMessageQueue.MessageQueueListener)registrations.remove( listener );
