...
import torrentlib.util.platform.win32.PluginEventListener;
import torrentlib.util.platform.win32.PluginManagerDefaults;
import torrentlib.util.platform.win32.PluginEvent;
import torrentlib.AETemporaryFileHandler;
import torrentlib.SystemProperties;
import torrentlib.DelayedEvent;
import stdlib.security.types.SHA1Simple;
import torrentlib.NonDaemonTaskRunner;
import torrentlib.ListenerManagerDispatcher;
import torrentlib.ListenerManager;
import connect.tracker.host.TRHostFactory;
import connect.tracker.host.TRHost;
import comm.ipfilter.IpFilterManagerFactory;
import torrentlib.internat.LocaleUtil;
import torrentlib.AzureusCoreLifecycleAdapter;
import torrentlib.AzureusCoreOperation;
import torrentlib.AzureusCoreOperationTask;
import torrentlib.AzureusCoreComponent;
import torrentlib.AzureusCoreRunningListener;
import torrentlib.AzureusCoreOperationListener;
import torrentlib.AzureusCoreLifecycleListener;
import torrentlib.AzureusCoreListener;
...
import controller.config.TransferSpeedValidator;
import torrentlib.disk.DiskManager;
import xfer.download.DownloadManagerState;
import controller.GlobalManagerAdapter;
import controller.GlobalManagerFactory;
import controller.GlobalManagerStats;
import controller.GlobalMangerProgressListener;
import comm.ipfilter.IpFilterManager;
import connect.peer.PEPeerManager;
import connect.peer.PEPeerSource;
import torrentlib.security.SESecurityManager;
...
import plugins.torrent.TorrentDownloader;
import plugins.ui.UIManager;
import plugins.ui.UIManagerEvent;
import plugins.utils.DelayedTask;
import plugins.utils.PowerManagementListener;
import plugins.utils.ScriptProvider;
import plugins.utils.StaticUtilities;
...
import torrentlib.customization.CustomizationManagerFactory;
import connect.dht.DHT;
import connect.dht.DHTListener;
import connect.dht.speed.DHTSpeedTester;
import torrentlib.instancemanager.AZInstanceManager;
import torrentlib.instancemanager.AZInstanceManagerAdapter;
import torrentlib.instancemanager.AZInstanceManagerFactory;
import torrentlib.instancemanager.AZInstanceTracked;
...
import controller.networkmanager.admin.NetworkAdminNetworkInterface;
import controller.networkmanager.admin.NetworkAdminNetworkInterfaceAddress;
import controller.networkmanager.admin.NetworkAdminPropertyChangeListener;
import controller.networkmanager.tcp.TCPNetworkManager;
import controller.networkmanager.udp.UDPNetworkManager;
import connect.peermanager.PeerManager;
import connect.peermanager.PeerNATTraverser;
import comm.proxy.AEProxySelectorFactory;
import plugins.clientid.ClientIDPlugin;
import torrentlib.security.CryptoManager;
import torrentlib.security.CryptoManagerFactory;
import torrentlib.speedmanager.SpeedManager;
import torrentlib.speedmanager.SpeedManagerAdapter;
import torrentlib.speedmanager.SpeedManagerFactory;
...
import torrentlib.util.PrimaryClassloader;
import plugins.dht.DHTPlugin;
import plugins.startstoprules.defaultplugin.DefaultRankCalculator;
import plugins.startstoprules.defaultplugin.StartStopRulesDefaultPlugin;
import plugins.tracker.dht.DHTTrackerPlugin;
...
TorrentEngineCore
{
	...
	protected static TorrentEngineCore		singleton;
	...
	public static TorrentEngineCore
	create()
		...
		throws AzureusCoreException
	{
		try{
			...
			singleton	= new TorrentEngineCore();
	...
	TorrentEngineCore()
	{
		...
		instance_manager =
			AZInstanceManagerFactory.getSingleton(new AZInstanceManagerAdapter()
				{
					public String
					getID()
					{
					...
					{
						TorrentEngineCore.this.addLifecycleListener(new AzureusCoreLifecycleAdapter()
							{
								public void
								started(
		...
		throws AzureusCoreException
	{
	   ...
	   if (sDelayCore != null) {
		...
		AEThread2 pluginload = new AEThread2("PluginLoader",true)
		{
			public void run() {
				...
				pi.loadPlugins(TorrentEngineCore.this, false, !"0".equals(System.getProperty("azureus.loadplugins")), true, true);
				if (Logger.isEnabled())
	   ...
	   Runtime.getRuntime().addShutdownHook(new AEThread("Shutdown Hook") {
	     public void runSupport() {
			...
			TorrentEngineCore.this.stop();
	     }
	   });


	   DelayedTask delayed_task =
	   		UtilitiesImpl.addDelayedTask("Core",
	   			new Runnable()
	   			{
	   				public void
	   				run()
	   				{
	   					new AEThread2( "core:delayTask", true )
	   					{
	   						...
	   						run()
	   						{
			   					...
			   					na.runInitialChecks(TorrentEngineCore.this);
	   ...
	   try {
		...
		new AEThread2("Plugin Init Complete", false )
		{
			...
			{
				try{
					PlatformManagerFactory.getPlatformManager().startup(TorrentEngineCore.this );
				...
				while( it.hasNext()){
					...
					try{
						...
						if ( !listener.requiresPluginInitCompleteBeforeStartedEvent()){
							...
							listener.started(TorrentEngineCore.this );
						}
					}catch( Throwable e ){
		...
		for (final AzureusCoreRunningListener l : runningListeners) {
			try {
				tp.run(new AERunnable() {
					public void runSupport() {
						l.azureusCoreRunning(TorrentEngineCore.this);
					}
				});
			} catch (Throwable t) {
				Debug.out(t);
		...
		throws AzureusCoreException
	{
		...
		try{
			...
			ListenerManager.dispatchWithTimeout(async_listeners,
					new ListenerManagerDispatcher()
					{
						...
						dispatch(
							...
							Object		value )
						{
							((AzureusCoreLifecycleListener)listener).stopping(TorrentEngineCore.this );
						}
					},
					10*1000 );
			...
			ListenerManager.dispatchWithTimeout(async_listeners,
					new ListenerManagerDispatcher()
					{
						...
						dispatch(
							...
							Object		value )
						{
							((AzureusCoreLifecycleListener)listener).stopped(TorrentEngineCore.this );
						}
					},
					10*1000 );
			...
			public void runSupport() {
				...
				while( it.hasNext()){
					...
					if (!((AzureusCoreLifecycleListener)it.next())
							.stopRequested(TorrentEngineCore.this)) {
						if (Logger.isEnabled())
	...
	checkRestartSupported()
		...
		throws AzureusCoreException
	{
		if ( getPluginManager().getPluginInterfaceByClass( "update.UpdaterPatcher") == null ){
...
//		}
		...
		AzureusCoreOperation	op =
			new AzureusCoreOperation()
			{
				...
				reportCurrentTask(
					String task )
				{
					TorrentEngineCore.this.reportCurrentTask( this, task );
				}
				...
				reportPercent(
					int percent )
				{
					TorrentEngineCore.this.reportPercent( this, percent );
				}
			};
	...
	public static void addCoreRunningListener(AzureusCoreRunningListener l) {
	   mon_coreRunningListeners.enter();
	   try {
    		if (TorrentEngineCore.coreRunningListeners != null) {
	   ...
	   l.azureusCoreRunning(TorrentEngineCore.getSingleton());
	}
