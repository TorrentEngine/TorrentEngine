...
import torrentlib.data.torrent.TOTorrentAnnounceURLGroup;
...
import torrentlib.util.DNSUtils;
...
import torrentlib.FileIsADirectoryException;
...
public class TorrentUtils {
    ...
    /**
     * If you set "create_delegate" to true then you must understand that this
     * results is piece hashes being discarded and then re-read from the torrent
     * file if needed Therefore, if you delete the original torrent file you're
     * going to get errors if you access the pieces after this (and they've been
     * discarded)
     *
     * @param file
        ...
        while (it1.hasNext()) {
            ...
            while (it2.hasNext()) {
                ...
                if (url.equals(NO_VALID_URL_URL)) {
                    ...
                    } else if (removeSet.contains(url)) {
    ...
    /**
     * Register keys that are used for heavyweight maps that should be discarded
     * when the torrent is not in use Make sure these keys are only ever used
     * for Map objects!
     */
    public static void
            registerMapFluff(
    ...
    public interface ExtendedTorrent
            extends TOTorrent {
    ...
    /**
     * Deletes the given dir and all dirs underneath if empty. Don't delete
     * default save path or completed files directory, however, allow deletion
     * of their empty subdirectories Files defined to be ignored for the sake of
     * torrent creation are automatically deleted For example, by default this
     * includes thumbs.db
     */
    ...
    /**
     * Same as #recursiveEmptyDirDelete(File), except allows disabling of
     * logging of any warnings
     *
    ...
    private static DNSTXTEntry
        ...
        if (is_new) {
            ...
            try {
                ...
                } else if (!old_txt_entry.sameAs(txt_entry)) {
