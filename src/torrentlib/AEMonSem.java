...
AEMonSem
{
	...
	private static ThreadLocal<Stack<AEMonSem>> tls	=
		new ThreadLocal<Stack<AEMonSem>>()
		{
			public Stack<AEMonSem>
			initialValue()
			{
				return( new Stack<AEMonSem>());
			}
		};
	...
	private static Map<String, String[]> 	debug_traces = new HashMap<String, String[]>();
	private static List<String>	debug_recursions	= new ArrayList<String>();
	private static List<String>	debug_reciprocals	= new ArrayList<String>();
	//private static List	debug_sem_in_mon	= new ArrayList();

	private static Map<String, monSemData> debug_name_mapping  = new WeakHashMap<String, monSemData>();
	private static Map<AEMonSem, monSemData> debug_monitors    = new WeakHashMap<AEMonSem, monSemData>();
	private static Map<AEMonSem, monSemData>	debug_semaphores = new WeakHashMap<AEMonSem, monSemData>();
		...
		AEDiagnosticsLogger diag_logger )
	{
		...
		List<AEMonitor> waiting_monitors	= new ArrayList<AEMonitor>();
		List<AEMonitor> busy_monitors	= new ArrayList<AEMonitor>();
		List<AEMonSem> waiting_semaphores = new ArrayList<AEMonSem>();
		...
		if ( waiting_semaphores.size() > 0 ){
			...
			for (int i = 0; i < waiting_semaphores.size(); i++){
				...
				AEMonSem	ms = waiting_semaphores.get(i);
